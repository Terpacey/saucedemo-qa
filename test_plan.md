...
- Requirements Traceability Matrix (`rtm.md`), linking all manual test cases, automated tests, and defect reports by feature area
- Automated test execution report (`automation/reports/report.html`), generated by pytest-html on each run; each run overwrites the previous report
