...
def test_item_count_standard_user(standard_user_session):
    page = InventoryPage(standard_user_session)
    assert len(page.get_item_names()) == 6


def test_item_names_standard_user(standard_user_session):
    page = InventoryPage(standard_user_session)
    assert page.get_item_names() == EXPECTED_NAMES


def test_item_prices_standard_user(standard_user_session):
    page = InventoryPage(standard_user_session)
...
def test_cart_badge_absent_when_empty_standard_user(standard_user_session):
    page = InventoryPage(standard_user_session)
    assert page.get_cart_badge_count() == 0


def test_add_to_cart_increments_badge_standard_user(standard_user_session):
    page = InventoryPage(standard_user_session)
    page.click_item_button("Sauce Labs Backpack")
    assert page.get_cart_badge_count() == 1


def test_remove_from_inventory_decrements_badge_standard_user(standard_user_session):
    page = InventoryPage(standard_user_session)
    page.click_item_button("Sauce Labs Backpack")
    assert page.get_cart_badge_count() == 1
    page.click_item_button("Sauce Labs Backpack")
...
def test_add_button_changes_to_remove_standard_user(standard_user_session):
    page = InventoryPage(standard_user_session)
...
def test_sort_standard_user(standard_user_session, sort_value, expected_first, expected_last):
    page = InventoryPage(standard_user_session)
...
def test_cart_navigation_standard_user(standard_user_session):
    page = InventoryPage(standard_user_session)
    page.click_cart()
...
def test_sort_triggers_alert_error_user(error_user_session, sort_value):
...
def test_only_three_items_addable_error_user(error_user_session):
    page = InventoryPage(error_user_session)
...
def test_remove_non_functional_on_inventory_error_user(error_user_session):
    page = InventoryPage(error_user_session)
    page.click_item_button("Sauce Labs Backpack")
    assert page.get_cart_badge_count() == 1
    page.click_item_button("Sauce Labs Backpack")
...
def test_sort_silently_broken_problem_user(problem_user_session):
...
def test_all_images_are_pug_problem_user(problem_user_session):
    page = InventoryPage(problem_user_session)
...
def test_only_three_items_addable_problem_user(problem_user_session):
    page = InventoryPage(problem_user_session)
    for name in EXPECTED_NAMES:
        page.click_item_button(name)
    assert page.get_cart_badge_count() == 3
...
def test_remove_non_functional_on_inventory_problem_user(problem_user_session):
    page = InventoryPage(problem_user_session)
    page.click_item_button("Sauce Labs Backpack")
    assert page.get_cart_badge_count() == 1
    page.click_item_button("Sauce Labs Backpack")
...
def test_add_remove_functional_visual_user(visual_user_session):
...
def test_prices_randomised_visual_user(visual_user_session):
    page = InventoryPage(visual_user_session)
    names = page.get_item_names()
    prices = page.get_item_prices()
    displayed = dict(zip(names, prices))
...
def test_items_correct_after_load_performance_glitch_user(performance_glitch_user_session):
