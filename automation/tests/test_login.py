...
def test_valid_login_standard_user(standard_user_session) -> None:
...
def test_valid_login_all_users(driver, login_page, username) -> None:
...
@pytest.mark.defect
def test_locked_out_user(driver, login_page) -> None:
...
def test_sql_injection_username(driver, login_page) -> None:
    login_page.login("' OR '1'='1", "secret_sauce")
    assert not login_page.is_on_inventory_page()


def test_sql_injection_password(driver, login_page) -> None:
